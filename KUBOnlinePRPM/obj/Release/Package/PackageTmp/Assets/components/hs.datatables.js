	...
	$.HSCore.components.HSDatatables = {
		...
		initDatatables: function() {
			...
			this.collection.each(function(i, el) {
				...
				$($info).html(
					'Showing 1 to ' + info.length + ' of ' + info.recordsTotal + ' Entries'
				);
				...
				$($entries).on('change', function() {
					...
					$($info).html(
						'Showing ' + val + ' to ' + info.length + ' of ' + info.recordsTotal + ' Entries'
                        //'Showing 1 to ' + info.length + ' of ' + info.recordsTotal + ' Entries'
					);
				});
		...
		pagination: function(target, table, pagiclasses, pagiitemclasses, pagilinksclasses, paginextclasses, paginextlinkclasses, paginextlinkmarkup, pagiprevclasses, pagiprevlinkclasses, pagiprevlinkmarkup, pages) {
			var info = table.page.info(),
				paginationMarkup = '';
			var pageNo = info.recordsTotal / info.length
			if (pageNo % 1 != 0) //true: it's not an integer, false: it's an integer
			    pageNo = Math.floor(pageNo) + 1;
            for (var i = 0; i < pageNo; i++) {
                paginationMarkup += i / pageNo == 0 ?
			        '<li class="' + pagiitemclasses + '"><a id="datatablePaginationPage' + i + '" class="' + pagilinksclasses + ' active" href="#!" data-dt-page-to="' + i + '">' + (i + 1) + '</a></li>'
			    :
                    '<li class="' + pagiitemclasses + '"><a id="' + target + i + '" class="' + pagilinksclasses + '" href="#!" data-dt-page-to="' + i + '">' + (i + 1) + '</a></li>';
            }
